  ...
  files.forEach(file => {
    ...
    if (path.extname(file) === '.md') {
      ...
      fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        // `categories` 값을 찾고, 소문자로 변환합니다.
        let updatedFrontMatter = frontMatter;
        const categoriesMatch = frontMatter.match(/categories:\s*\[([^\]]+)\]/);

        if (categoriesMatch) {
          // categories 부분을 찾아 소문자로 변환
          const categories = categoriesMatch[1]
            .split(',')
            .map(category => category.trim().toLowerCase())
            .join(', ');

          // 소문자로 변환된 `categories`를 Front Matter에 적용
          updatedFrontMatter = frontMatter.replace(categoriesMatch[0], `categories: [${categories}]`);
        }
        ...
        fs.writeFile(filePath, updatedContent, 'utf8', (err) => {
          ...
          console.log(`${file}의 categories를 소문자로 변환했습니다.`);
        });
      });
    }
  });
